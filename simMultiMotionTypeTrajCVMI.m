function [xCoordMat,yCoordMat,trajClass,errFlag] = ...
    simMultiMotionTypeTrajCVMI(numParticles,volumeEdges,totalTime,timeStep,...
    diffCoefRange,confRadRange,driftVelRange,durationRange,strictSwitch)
%simMultiMotionTypeTrajCVMI generates trajectories that exhibit a combination of confined diffusion, free diffusion and diffusion with drift
%
%SYNOPSIS [xCoordMat,yCoordMat,trajClass,errFlag] = ...
%    simMultiMotionTypeTrajCVMI(numParticles,volumeEdges,totalTime,timeStep,...
%    diffCoefRange,confRadRange,driftVelRange,durationRange,strictSwitch)
%
...
%
%OUTPUT xCoordMat  : (Number of particles) - by - (number of time points)
%                     matrix of particle x-coordinates. NaN indicates that
%                     particle does not exist at particular time point.
%       yCoordMat  : (Number of particles) - by - (number of time points)
%                     matrix of particle y-coordinates. NaN indicates that
%                     particle does not exist at particular time point.
%       trajClass  : (Number of particles) - by - 1 cell array. Each entry
...
%                    the end (say frame 100). Its trajClass = [1 50 2; 51 100 1].
...
if nargin < nargin('simMultiMotionTypeTrajCVMI')
    disp('--simMultiMotionTypeTrajCVMI: Incorrect number of input arguments!');
    errFlag = 1;
    return
end
...
if numMotionTypes == 1 && strictSwitch == 1 && maxPossibelSegments > 1
    disp('--simMultiMotionTypeTrajCVMI: Inconsistency - please make motion duration longer than totalTime, or make strictSwitch = 0!');
    errFlag = 1;
    return
end
...
%output
xCoordMat = trackMatrix(:,1:8:end);
yCoordMat = trackMatrix(:,2:8:end);
trajClass = cell(numParticles,1);
for iPart = 1 : numParticles
    trajClass{iPart} = trajTransDiffClass(iPart).segmentClass.momentScalingSpectrum(:,1:3);
end
